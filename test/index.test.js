...
describe('splitReasoningFromString', () => {
  test('separates reasoning content from visible text', () => {
    const { text, reasoning } = splitReasoningFromString('hi <thinking>secret</thinking> there', {
      startTag: '<thinking>',
      endTag: '</thinking>',
      keepTags: false,
    });
...
describe('createThinkingMiddleware', () => {
  test('strips reasoning from text responses and aggregates deltas', async () => {
    ...
    expect(parts).toStrictEqual([
      ...
      { type: 'reasoning', reasoning: { type: 'text-delta', textDelta: 'hid' } },
      { type: 'reasoning', reasoning: { type: 'text-delta', textDelta: 'den' } },
  ...
  test('keeps thinking tags when stripReasoningFromText is false', async () => {
    const middleware = createThinkingMiddleware({ stripReasoningFromText: false });
  ...
  test('works end-to-end with ReadableStream inputs', async () => {
    ...
    const upstream = new ReadableStream({
      start(controller) {
        controller.enqueue({ type: 'text-delta', textDelta: 'Hi <thinking>calc' });
        controller.enqueue({ type: 'text-delta', textDelta: ' reasoning</thinking> there' });
        controller.close();
      },
    });
    ...
    expect(emitted).toStrictEqual([
      ...
      { type: 'reasoning', reasoning: { type: 'text-delta', textDelta: 'calc' } },
      { type: 'reasoning', reasoning: { type: 'text-delta', textDelta: ' reasoning' } },
